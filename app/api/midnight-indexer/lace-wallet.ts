...
export async function connectLaceWallet(options: Record<string, unknown> = {}): Promise<LaceWalletSession> {
  ...
  for (const candidate of providerCandidates) {
    ...
    try {
      ...
      let api: LaceWalletApi

      if (candidate.source === "midnight") {
        const midnightApi = candidate.provider as any

        // 🔐 pedir permiso explícito
        await midnightApi.request({ method: "wallet_enable" })
      
        api = midnightApi
      } else {
        api = await enableWithTimeout(
          candidate.provider.enable(enableOptions),
          candidate.source
        )
      }
      ...
      if (!address && api.getChangeAddress) {
     ...
     // const address = extractAddress(api, stateValue)
      ...
      if (candidate.source === "midnight") {
        if (!coinPublicKey || !encryptionPublicKey) {
          throw new Error("Failed to obtain Midnight public keys from Lace API")
        }
      }
      if (networkId === null) {
        networkId = 0
